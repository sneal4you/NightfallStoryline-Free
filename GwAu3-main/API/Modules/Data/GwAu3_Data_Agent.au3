...
Func Agent_GetAgentsAsObstacles($a_f_Range = 2500, $a_f_Radius = 150, $a_s_Filter = "", $a_i_RefAgentID = -2)
    ...
    Local $l_a_AgentArray = Agent_GetAgentArray() 
...
Func VisibleEffect_GetAll($a_i_AgentID = -2)
    ...
    While $l_p_Current <> 0 And $l_i_Count < 100
        ...
        $l_a_Effects[$l_i_Count][1] = Memory_Read($l_p_Current, "dword") 
        $l_a_Effects[$l_i_Count][2] = Memory_Read($l_p_Current + 0x4, "dword") 
        $l_a_Effects[$l_i_Count][3] = Memory_Read($l_p_Current + 0x8, "dword") 
        If Not Utils_TList_Iterator_Next($l_a_Iterator) Then ExitLoop
        $l_p_Current = Utils_TList_Iterator_Current($l_a_Iterator)
    WEnd
...
Func VisibleEffect_FindByID($a_i_AgentID = -2, $a_i_EffectID = 0)
    ...
    While $l_p_Current <> 0
        ...
        If $l_i_ID = $a_i_EffectID Then
            ...
            $l_a_Result[2] = Memory_Read($l_p_Current, "dword") 
            $l_a_Result[3] = Memory_Read($l_p_Current + 0x8, "dword") 
            Return $l_a_Result
        EndIf
        If Not Utils_TList_Iterator_Next($l_a_Iterator) Then ExitLoop
        $l_p_Current = Utils_TList_Iterator_Current($l_a_Iterator)
...
Func _ApplyFilters($a_p_Agent, $a_s_Filters)
	...
	Local $l_as_Filters = StringSplit($a_s_Filters, "|", 2) 
	For $i = 0 To UBound($l_as_Filters) - 1
		Local $l_s_FilterName = StringStripWS($l_as_Filters[$i], 3) 
