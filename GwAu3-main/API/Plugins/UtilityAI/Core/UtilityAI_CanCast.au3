...
Func UAI_CanUse($a_i_SkillSlot)
	...
	If $l_i_BaseSacrificeCost <> 0 Then
		...
		If UAI_PlayerHasEffect($GC_I_SKILL_ID_Awaken_the_Blood) Then $l_i_TotalHealthCost = $l_i_TotalHealthCost + ($l_i_TotalHealthCost * 0.5) 
		If UAI_PlayerHasEffect($GC_I_SKILL_ID_Scourge_Sacrifice) Then $l_i_TotalHealthCost = $l_i_TotalHealthCost * 2 
	EndIf
	...
	If UAI_PlayerHasEffect($GC_I_SKILL_ID_Quickening_Zephyr) Then $l_i_EnergyCost *= 1.3 
	Switch $l_i_SkillType
		Case $GC_I_SKILL_TYPE_HEX
			If UAI_PlayerHasEffect($GC_I_SKILL_ID_Natures_Renewal) Then $l_i_EnergyCost *= 2 
		...
		Case $GC_I_SKILL_TYPE_SIGNET
			If UAI_PlayerHasEffect($GC_I_SKILL_ID_Primal_Echoes) Then $l_i_EnergyCost += 10 
		Case $GC_I_SKILL_TYPE_CHANT, $GC_I_SKILL_TYPE_SHOUT
			If UAI_PlayerHasEffect($GC_I_SKILL_ID_Roaring_Winds) Then _
				$l_i_EnergyCost += UAI_GetPlayerEffectInfo($GC_I_SKILL_ID_Roaring_Winds, $GC_UAI_EFFECT_Scale) 
		Case $GC_I_SKILL_TYPE_SPELL
			...
			If UAI_PlayerHasEffect($GC_I_SKILL_ID_Attuned_Was_Songkai) Then _
				$l_i_EnergyCost -= ($l_i_BaseEnergyCost * UAI_GetPlayerEffectInfo($GC_I_SKILL_ID_Attuned_Was_Songkai, $GC_UAI_EFFECT_Scale)) 
		Case $GC_I_SKILL_TYPE_RITUAL
			If UAI_PlayerHasEffect($GC_I_SKILL_ID_Attuned_Was_Songkai) Then _
				$l_i_EnergyCost -= ($l_i_BaseEnergyCost * UAI_GetPlayerEffectInfo($GC_I_SKILL_ID_Attuned_Was_Songkai, $GC_UAI_EFFECT_Scale)) 
			If UAI_PlayerHasEffect($GC_I_SKILL_ID_Soul_Twisting) Then $l_i_EnergyCost -= 15 
		Case $GC_I_SKILL_TYPE_WEAPON_SPELL, $GC_I_SKILL_TYPE_ITEM_SPELL
			If UAI_PlayerHasEffect($GC_I_SKILL_ID_Renewing_Memories) Then _
				$l_i_EnergyCost -= ($l_i_BaseEnergyCost * UAI_GetPlayerEffectInfo($GC_I_SKILL_ID_Renewing_Memories, $GC_UAI_EFFECT_Scale)) 
	EndSwitch
	If UAI_PlayerHasEffect($GC_I_SKILL_ID_Energizing_Wind) Then $l_i_EnergyCost -= 15 
	Switch $l_i_Attribute
		Case $GC_I_ATTRIBUTE_HEALING_PRAYERS
			If UAI_PlayerHasEffect($GC_I_SKILL_ID_Divine_Spirit) Then $l_i_EnergyCost -= 5 
			If UAI_PlayerHasEffect($GC_I_SKILL_ID_Healers_Covenant) Then $l_i_EnergyCost -= 3 
		...
		Case $GC_I_ATTRIBUTE_BLOOD_MAGIC To $GC_I_ATTRIBUTE_SOUL_REAPING
			If UAI_PlayerHasEffect($GC_I_SKILL_ID_Cultists_Fervor) Then _
				$l_i_EnergyCost -= UAI_GetPlayerEffectInfo($GC_I_SKILL_ID_Cultists_Fervor, $GC_UAI_EFFECT_Scale) 
	EndSwitch
	...
	If ($l_i_Combo = $GC_I_SKILL_COMBO_OFF_HAND_ATTACK Or $l_i_Combo = $GC_I_SKILL_COMBO_DUAL_ATTACK) Then
		...
		If $l_b_WotEP Then $l_i_EnergyCost = 0 
	EndIf
...
Func UAI_CanDrop($a_i_SkillSlot)
	Switch UAI_GetStaticSkillInfo($a_i_SkillSlot, $GC_UAI_STATIC_SKILL_SkillID)
		Case $GC_I_SKILL_ID_ANGUISHED_WAS_LINGWAH
			...
			For $i = 1 To 8
				If $i = $a_i_SkillSlot Then ContinueLoop 
		...
		Case $GC_I_SKILL_ID_LIVELY_WAS_NAOMEI
			...
			If $l_i_DeadAlly = 0 Then Return False 
